...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Force header early to prevent errors
    ...
    try {
        ...
        $resetUrl = "http://" . $_SERVER['HTTP_HOST'] . "/ai-business-analytics-system/frontend/templates/reset-password.php?token={$token}&email=" . urlencode($email);
        ...
        $mail->Password   = 'YOUR_MAILTRAP_PASSWORD_HERE'; 
        $mail->Port       = 2525;
        $mail->setFrom('no-reply@analytiq.com', 'ANALYTIQ');
        $mail->addAddress($email);
        $mail->isHTML(true);
        $mail->Subject = 'Reset Your Password';
        $mail->Body    = "Hi, click here to reset your password: <a href='{$resetUrl}'>{$resetUrl}</a>";

        $mail->send();

        echo json_encode(['success' => true, 'message' => 'A reset link has been sent.']);
    } catch (Exception $e) {
        // Send the error message back to the JS so you can debug it
